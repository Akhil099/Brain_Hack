...
test_mse_loss = Flux.Losses.mse(predictions, val_labels)
println("Mse loss on test_data: ", test_mse_loss)

println("Huber_loss loss in the test_data: ", Flux.Losses.huber_loss(predictions, val_labels))
println("mse loss in the test_data: ", Flux.Losses.mae(predictions, val_labels))